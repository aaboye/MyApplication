...
public class MyBanner extends FrameLayout {
    ...
    int[] resIds;//viewPage管理的所有图片（用户看到的图片+2）

    public MyBanner(Context context, int[] ids) {
        super(context);
        if (ids != null && ids.length > 0) {
            //创建mybanner时，创建者传入的要现实的轮播图片
    ...
    public void OnCloseBannerListener(OnCloseBannerListener listener){
    ...
    private void initView(View view) {
        viewPager = (ViewPager) view.findViewById(R.id.viewPage_banner);
        ivClose = (ImageView) view.findViewById(R.id.image_banner);
